...
{
    public RectTransform baseTimon;   // La base del tim�n
    public RectTransform mango;        // El tim�n que vamos a girar

    public float velocidadMinima = 100f;  // velocidad base de regreso
    public float factorGrados = 2f;       // cu�nto aumenta la velocidad seg�n cu�nto giraste

    public static bool estoyTocando = false;
    [SerializeField] public float angulo = 0;           // �ngulo actual
    private float gradosAcumulados = 0; // cu�nto gir� desde que agarre el tim�n
    private float anguloAnterior;       // para guardar el �ngulo del frame anterior
    public static bool IsTouchingJoystick { get; private set; } // para saber si está tocando el joystick

    // Cuando empiezo a tocar
    public void OnPointerDown(PointerEventData e)
    {
        estoyTocando = true;
        anguloAnterior = CalcularAngulo(e); // guardo el �ngulo inicial
    }
    ...
    public void OnDrag(PointerEventData e)
    {
        ...
        // calculo �ngulo actual
        float anguloActual = CalcularAngulo(e);

        // diferencia entre el �ltimo y este
